 ...
 useEffect(()=>{
  ...
  return (
    <div className='flex min-h-screen bg-gray-100'>
      ...
      <div className='flex felx-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-3'>

        <div className='flex items-center gap-3'>
          <FaLongArrowAltLeft className='w-[22px] h-[22px] cursor-pointer' onClick={()=>navigate('/dashboard')}/>
          <h1 className='text-2xl font-semibold'>All Created Coures</h1>
        </div>
      ...
      <div className='hidden md:block bg-white rounded-xl shadow p-4 overflow-x-auto'>
        ...
        <p className='text-center text-sm text-gray-400 mt-6'>A list of your recent coures</p>
      </div>
     ...
     className='text-gray-600 hover:text-blue-600 cursor-pointer'
      ...
      />
          ...
          <span className={` w-fit px-3 py-1 rounded-full text-xs ${course?.isPublished?" bg-green-100 text-green-600":"bg-red-100 text-red-600"}`}>{course?.isPublished?"Published":"Draft"}</span>
        </div>))}
