    ...
    <div className='min-h-screen flex items-center justify-center bg-gray-100 px-4'>
      ...
      {step==1 &&(<div className='bg-white shadow-md rounded-xl p-8 max-w-md w-full'>
        ...
        <form className='space-y-4' onSubmit={(e)=>e.preventDefault()}>
          ...
          <input id='email' type="text" className='mt-1 w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-[black]' placeholder='your@example.com' onChange={(e)=>setEmail(e.target.value)} value={email}/>
          </div>
      ...
      {/* step 2 */}
        ...
        {/* step 3 */}
        {step==3 &&(<div className='bg-wihte shadow-md rounded-xl p-8 max-w-md w-full'>
        <h2 className='text-2xl font-bold mb-6 text-center text-gray-800'>Reset Your Password</h2>
        <p className='text-sm text-gray-500 text-center m-6'>Enter a new password below to regain access to your account.</p>
        <form className='space-y-4' onSubmit={(e)=>e.preventDefault()}>
          <div>
          <label htmlFor='password' className='block text-sm font-medium text-gray-700'>New Password</label>
          <input id='password' type="password" className='mt-1 w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-[black]' placeholder='***********' onChange={(e)=>setNewPassword(e.target.value)} value={newpassword}/>
          </div>
           <div>
          <label htmlFor='conpassword' className='block text-sm font-medium text-gray-700'>Confirm Password</label>
          <input id='conpassword' type="password" className='mt-1 w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-[black]' placeholder='***********' onChange={(e)=>setConPassword(e.target.value)} value={conPassword}/>
          </div>
