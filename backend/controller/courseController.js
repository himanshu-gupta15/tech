...
import uploadCloudinary from '../config/cloudinary.js' // adjust path
...
export const getPublishedCourses=async(req,res)=>{
   try{
        ...
        const courses = await Course.find({ isPublished: true }).populate("lectures reviews");
         if(!courses){
          return res.status(400).json({message:"Coures is not found"})
         }
...
export const getCreatorCourses=async(req,res)=>{
  try{
       ...
       if(!courses){
          return res.status(400).json({message:"Coures are not found"})
         }
...
export const getCourseById=async(req,res)=>{
  ...
  //  let course=await Course.findById(courseId)
    ...
    .populate({
        path: "reviews",
        populate: { path: "user", select: "name photoUrl role" }, // bring review.user details
      });
   if(!course){
...
//   }catch(error){
//      return res.status(500).json({message:`failed to create Course by id ${error}`})
//   }
// }
...
export const createLecture = async (req, res) => {
  try {
    ...
    const updatedCourse = await Course.findById(courseId).populate("lectures");
...
export const getCreatorById=async(req,res)=>{
  try{  const {userId}=req.body
   ...
   if(!user){
    return res.status(404).json({message:"User is no Found"})
   }
